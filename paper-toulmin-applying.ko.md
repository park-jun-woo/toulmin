...
vw := votingGraph.Rule(IsAdult)
vr := votingGraph.Counter(HasCriminalRecord)
vr.Attacks(vw)

contractGraph := toulmin.NewGraph("contract")
cw := contractGraph.Rule(IsAdult)
cr := contractGraph.Counter(IsBankrupt)
cr.Attacks(cw)
```
...
verdict는 **주장의 성립 정도**다. +쪽이 긍정(주장이 지지됨), -쪽이 부정(주장이 반박됨)이다. 엔진은 값만 산출하며, 도메인별 해석은 사용자의 몫이다. 직관에 부합하려면 규칙의 주장을 긍정형으로 설계하는 것을 권장한다 — "접근 허용"(+1 = 허용), "파일 구조 준수"(+1 = 준수). 부정형("위반 감지") 규칙도 유효하지만, 이 경우 +1이 위반을 의미하게 되어 해석이 반직관적이다.

### 4.5 Evaluate와 EvalOption
