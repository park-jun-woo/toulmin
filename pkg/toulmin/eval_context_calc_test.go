...
//ff:what TestEvalContextCalc — tests evalContext.calc's h-Categoriser branches: prior error, cache hit, missing/nil fn, rule panic, already-ran skip, inactive, Strict vs Defeasible attacker loop
package toulmin
...
func TestEvalContextCalc(t *testing.T) {
	cases := []struct {
		name string
		...
		{"MissingFn", func(t *testing.T) {
			...
			if v := ec.calc("missing", NewContext()); v != -1.0 {
				t.Fatalf("expected -1.0, got %v", v)
			}
		}},
		...
		{"RulePanicSetsErr", func(t *testing.T) {
		...
		{"AlreadyRanInactive", func(t *testing.T) {
			// fn would fail the test if called again; already-ran must skip re-invocation.
			calledAgain := false
