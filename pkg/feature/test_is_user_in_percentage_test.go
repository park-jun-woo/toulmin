...
func TestIsUserInPercentage(t *testing.T) {
	...
	got1, _ := IsUserInPercentage(ctx1, toulmin.Specs{&PercentageSpec{Percentage: 1.0}})
	if !got1 {
	...
	got2, _ := IsUserInPercentage(ctx2, toulmin.Specs{&PercentageSpec{Percentage: 0.0}})
	if got2 {
	...
	got3, _ := IsUserInPercentage(ctx3, toulmin.Specs{&PercentageSpec{Percentage: h + 0.01}})
	ctx4 := toulmin.NewContext()
	ctx4.Set("id", "user-123")
	got4, _ := IsUserInPercentage(ctx4, toulmin.Specs{&PercentageSpec{Percentage: h + 0.01}})
	if got3 != got4 {
