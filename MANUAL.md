...
| `pkg/tangl` | TANGL v0.3 markdown policy language: runtime, ast, parser, validate, effects, gen | see "TANGL — Markdown Policy Language" below |
...
## TANGL — Markdown Policy Language

TANGL v0.3 compiles a markdown document straight into a `toulmin` defeats
graph plus saga-style execution. The old `pkg/tangl` prototype parser/validate
has been fully replaced by the pipeline below (`pkg/tangl` is now the
runtime; parsing/validation/codegen live in dedicated subpackages).

### Document Shape (7 sections, in order)

| Section | Purpose |
|---|---|
| `tangl:Subject` | names the document (`this document is \`name\``) |
| `tangl:See` | imports (`see \`alias\` from \`pkg/path\``) |
| `tangl:Definitions` | constants and struct-shaped types used by `with`/`using` |
| `tangl:Rules` | named inline rule expressions reusable across cases |
| `tangl:Cases` | judgment graphs: nodes (general/counter/except rules), `don't` attack edges, and attached `do`/`undo`/`run` execution edges |
| `tangl:Provides` | external entry points (endpoints) that `run` or `check` cases |
| `tangl:Internal` | event/tick-driven triggers with no external caller |

### Evaluate/Run Duality

A `Provides` or `Internal` entry either `check`s a case — compiles to
`Graph.Evaluate` (pure judgment, no side effects) — or `run`s a case —
compiles to `Graph.Run` (judgment + `RunOn`-driven execution). A `do` edge
fires when its node goes Active; `once` guards it (`tangl.OnceDone` /
`tangl.OnceMark`) so a retried tick or request does not repeat the action.
Its paired `undo` is pushed onto a saga compensation stack
(`tangl.PushCompensation`) at the moment the `do` succeeds, and is replayed
LIFO (`tangl.Compensate`) only if a later step in the same pass fails. A
fully successful pass calls `tangl.CommitCompensation` to discard the stack;
an unrecoverable failure escalates cause + compensation error together via
`tangl.Review` as a `*tangl.ReviewError`.

### Effect Summary

`pkg/tangl/effects.Closure` statically walks an endpoint's `run` case graph
(following `run <case> when <node>` edges in document order) and returns the
ordered `do`/`undo` actions reachable from it — an audit trail of "what can
this endpoint actually do" without executing anything. `checking` edges
(Evaluate-only, no side effects) are excluded from the closure.

### Key API by Package

| Package | Role | Key API |
|---|---|---|
| `pkg/tangl` | runtime used by generated code: saga compensation + once guards | `Required(ctx, fields...) error`, `ErrRequired`, `InitCompensation(ctx)`, `PushCompensation(ctx, fn)`, `Compensate(ctx) error`, `CommitCompensation(ctx)`, `OnceDone(ctx, key) bool`, `OnceMark(ctx, key)`, `Review(ctx, cause, comp) error`, `ReviewError{Cause, Compensate}` |
| `pkg/tangl/ast` | v0.3 AST types | `Document`, `Case`, `Node`, `Exec`, `Endpoint`, `Internal`, `Attack`, `Definition`, `Ref`, `Role` (General/Counter/Except) |
| `pkg/tangl/parser` | markdown → AST | `Parse(path string) (*ast.Document, error)`, `ParseSource(src, path string) (*ast.Document, error)` |
| `pkg/tangl/validate` | structural checks + advisories | `Validate(doc *ast.Document) error` (duplicate names, dangling refs, undo-without-do, checking/run/attack cycles — collects all violations), `Lint(doc *ast.Document) []string` (warns on unguarded `do` reachable from `tangl:Internal`) |
| `pkg/tangl/effects` | static effect-summary analyzer | `Closure(doc *ast.Document, endpoint string) ([]Entry, error)`; `Entry{Kind, Func, Once, Case, Node}` |
| `pkg/tangl/gen` | AST → Go source | `Generate(doc *ast.Document) (string, error)` — definitions, inline rules, case graph builders (`toulmin.NewGraph` + `RunOn`/`Run`), provided endpoints, internal triggers, `gofmt`-ed |
| `pkg/tangl/types` | base field types for `Definitions` | `Valid(typeName string, v any) bool` — text/number/integer/boolean/date/time/currency/quantity/email/url |
...
$ tangl gen pkg/tangl/parser/testdata/transfer.md | head -10
// Code generated by tangl. DO NOT EDIT.

package transfer

import (
	bank "bank/core"
	tangl "github.com/park-jun-woo/toulmin/pkg/tangl"
	log "github.com/park-jun-woo/toulmin/pkg/tangl/log"
	toulmin "github.com/park-jun-woo/toulmin/pkg/toulmin"
)
```
...
tangl check <file.md>                         # parse + validate + lint; prints "ok" or errors, lint warnings go to stderr
tangl ast <file.md>                           # parse and print the AST as indented JSON
tangl effects <file.md> <endpoint>            # print the static do/undo effect summary reachable from <endpoint>
tangl effects <subject>.<endpoint> --dir DIR  # same, resolving <subject> by scanning DIR for a matching document (default DIR ".")
tangl gen <file.md> [-o out.go]               # generate Go source for <file.md> (stdout, or -o/--out path)
```
