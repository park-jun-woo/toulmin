...
Warrant/Rebuttal/Defeater는 `*Rule`을 반환한다. 체이닝 없이 개별 문장으로 선언한다.
Defeat는 `*Rule` 참조만 받는다. backing이 다른 같은 함수도 `*Rule` 참조로 구분되므로 별도 API가 필요 없다.
...
blocked := g.Rebuttal(policy.IsIPInList, blockedIPs, 1.0)
rateLimited := g.Rebuttal(policy.IsRateLimited, nil, 1.0)
allowed := g.Defeater(policy.IsIPInList, whitelistedIPs, 1.0)
adminOverride := g.Defeater(policy.IsAdminOverride, nil, 1.0)
g.Defeat(blocked, auth)
g.Defeat(rateLimited, auth)
g.Defeat(allowed, blocked)
g.Defeat(adminOverride, rateLimited)
```

같은 `IsIPInList` 함수가 blockedIPs/whitelistedIPs 두 backing으로 사용된다. 각각 별도의 `*Rule`로 반환되므로 Defeat에서 `*Rule` 참조로 구분한다.
...
- `pkg/policy/request_context.go`: RequestContext, User 구조체 (런타임 데이터만 포함, 판정 기준은 backing으로 분리)
...
- Defeat 시나리오: 같은 함수 + 다른 backing 간 defeat 동작 검증 (`*Rule` 참조로 구분)
...
6. Defeat로 `*Rule` 참조를 전달하여 같은 함수 + 다른 backing 간 defeat edge가 정확히 동작한다
7. Warrant/Rebuttal/Defeater가 `*Rule`을 반환하고 체이닝 없이 개별 문장으로 사용된다
...
- Phase 010: backing 일급 시민 (3-element 시그니처, funcID+backing 식별)
- Phase 012: no-chaining API (`*Rule` 반환, `Defeat(*Rule, *Rule)`, `Graph` 타입명)
