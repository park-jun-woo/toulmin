...
클로저가 없다. 모든 rule은 순수 함수이며, 판정 기준은 `Warrant`/`Rebuttal`/`Defeater`의 backing 인자로 전달한다. backing이 필요 없는 rule은 `nil`을 명시한다.

```go
classifier := myClassifier{}  // Classifier 인터페이스 구현체

g := toulmin.NewGraph("post:publish").
    Warrant(moderate.IsVerifiedUser, nil, 1.0).
    Rebuttal(moderate.ContainsHateSpeech, classifier, 1.0).
    Rebuttal(moderate.ContainsSpam, classifier, 0.8).
    Rebuttal(moderate.ContainsNSFW, classifier, 1.0).
    Defeater(moderate.IsNewsContext, nil, 1.0).
    Defeater(moderate.IsEducational, nil, 1.0).
    Defeater(moderate.IsTrustedUser, 0.9, 1.0).
    Defeater(moderate.IsAdultChannel, nil, 1.0).
    Defeat(moderate.ContainsHateSpeech, moderate.IsVerifiedUser).
    Defeat(moderate.ContainsSpam, moderate.IsVerifiedUser).
    Defeat(moderate.ContainsNSFW, moderate.IsVerifiedUser).
    Defeat(moderate.IsNewsContext, moderate.ContainsHateSpeech).
    Defeat(moderate.IsEducational, moderate.ContainsHateSpeech).
    DefeatWith(moderate.IsTrustedUser, 0.9, moderate.ContainsSpam, classifier).
    Defeat(moderate.IsAdultChannel, moderate.ContainsNSFW)
...
backing이 있는 rule 간 defeat에는 `DefeatWith`를 사용한다. `IsTrustedUser`의 backing은 `0.9`(최소 신뢰 점수), `ContainsSpam`의 backing은 `classifier`이므로 각각을 명시해야 한다.
...
9. backing이 있는 rule 간 defeat에 DefeatWith가 정상 동작한다
...
- Phase 010: backing 일급 시민 (3-element 시그니처, Warrant/Rebuttal/Defeater backing 인자, DefeatWith, TraceEntry backing)
